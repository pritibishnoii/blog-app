...
export default async function Home({ searchParams }) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">{tag ? `Posts tagged #${tag}` : 'Latest posts'}</h1>
        <p className="text-gray-400">Read, write, and share — no heavy CMS required.</p>
      </div>
      ...
      {serialized.length === 0 ? (
        <div className="text-center py-24 text-gray-500">
          <p className="text-lg mb-1">No posts {tag ? `tagged #${tag}` : 'yet'}.</p>
